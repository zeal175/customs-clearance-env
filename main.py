...
# Re-exported for baseline.py compatibility
...
def evaluate_all_tasks(client, env_base_url: str, model: str) -> list:
    """Sync REST fallback used by baseline.py."""
    import requests as _req
    out: list[BaselineResult] = []
    for task_id in ("task1", "task2", "task3"):
        try:
            r = _req.post(f"{env_base_url.rstrip('/')}/reset", json={"task_id": task_id}, timeout=120)
            r.raise_for_status()
            obs = r.json()
            try:
                action = _llm_action(client, obs, task_id)
            except Exception:
                action = {"hs_code": "8471.30.00", "flags": [], "recommendation": "hold",
                          "confidence": 0.5, "assessable_value_inr": 100000.0,
                          "duty_amount_inr": 10000.0, "task_id": task_id, "metadata": {}}
            sr = _req.post(f"{env_base_url.rstrip('/')}/step", json=action, timeout=120)
            sr.raise_for_status()
